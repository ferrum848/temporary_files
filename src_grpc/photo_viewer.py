    ...
    def mousePressEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton:
            if self.image is not None:
                ...
                image = wand_pb2.Mask(image = image_bytes)
                x = wand_pb2.Mask(x = cursor_coord_x)
                y = wand_pb2.Mask(y = cursor_coord_y)
                wand = wand_pb2.Mask(wand = wand)
                antialiasing = wand_pb2.Mask(antialiasing=antialiasing)
                edges = wand_pb2.Mask(edges=edges)
                threshold = wand_pb2.Mask(threshold=threshold)
                mode = wand_pb2.Mask(mode=mode)
                criterion = wand_pb2.Mask(criterion=criterion)
                shape0 = wand_pb2.Mask(shape0=shape0)
                shape1 = wand_pb2.Mask(shape1=shape1)
                shape2 = wand_pb2.Mask(shape2=shape2)
                #result_image_bytes = stub.find_mask(image, x, y, wand, antialiasing, edges, threshold, mode, criterion, shape0, shape1, shape2)
                result_image_bytes = stub.find_mask(x)
                self.image = np.frombuffer(result_image_bytes, dtype=np.uint8).reshape(self.image.shape)
                # ============================================================================================
                '''
                if self.window.replace.isChecked():
